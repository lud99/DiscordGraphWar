...
const nextTurn = () => {
    ...
    } while (soldiers[currentTurn].dead) {};
...
class Obstacle {
    ...
    draw() {
        ...
        this.carvedCircles.forEach(hole => hole.draw());

       // ctx.drawImage(this.image, pos.x, pos.y, this.image.width * this.imageSizeFactor, this.image.height * this.imageSizeFactor);
    }

    isPointInside(x, y, extra = 0) {
        const collision = (x - this.x)**2 + (y - this.y)**2 <= (this.radius + extra)**2;
...
for (let x = MinX() + obstaclePadding; x <= MaxX() - obstaclePadding; x += obstacleSpacing) {
    //obstacles.push(new Obstacle(5, 0, 5));break;
...
const placePlayers = () => {
    //soldiers.push(new Soldier(-5, 0, 0));return;
...
const drawBg = () => {
    ...
    //obstacles.forEach(obstacle => obstacle.carvedCircles.forEach(c => c.draw));
